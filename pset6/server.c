...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char* spversion = strchr(http_version, ' ');
            char* terminator = strstr(http_version, "\r\n");
            char version[strlen(http_version) - strlen(terminator) + 1];
            strncpy(version, http_version, strlen(http_version) - strlen(terminator));
            version[strlen(http_version) - strlen(terminator)] = '\0';
            char met[strlen(line) - strlen(request_target)];
            strncpy(met, method, strlen(line) - strlen(request_target) - 1);
            met[strlen(line) - strlen(request_target) - 1] = '\0';
            char* spmet = strchr(met, ' ');
            char target[strlen(request_target) - strlen(http_version)];           
            strncpy(target, request_target, strlen(request_target) - strlen(http_version));
            target[strlen(request_target) - strlen(http_version) - 1] = '\0';
            char* sptarget = strchr(target, ' ');
            char* subquery = strchr(target, '?');
            
            
            
            char* qtarget = strchr(target, '"');
            
            char absolute_path[(subquery == NULL) ? strlen(target) + 1 : strlen(target) - strlen(subquery) + 1];
            strncpy(absolute_path, target, (subquery == NULL) ? strlen(target) : strlen(target) - strlen(subquery));
            absolute_path[(subquery == NULL) ? strlen(target) : strlen(target) - strlen(subquery) ] = '\0';
            char* dotpath = strchr(absolute_path, '.');
            
            
            if(method == NULL || qtarget != NULL || http_version == NULL || terminator == NULL)
            {
                error(400);
                continue;
            }
            if(spmet != NULL || sptarget != NULL || spversion !=NULL)
            {
                error(400);
                continue;
            }
            ...
            if(request_target == NULL || dotpath == NULL)
            {
                error(501);
                continue;
            }
            ...
            char query[(subquery == NULL) ? 1 : (strlen(target) - strlen(absolute_path))];
            memcpy(query, (subquery == NULL) ? http_version - 2 : subquery + 1, 
             (strlen(target) - strlen(absolute_path) <= 1) ? 1 : strlen(subquery));
            ...
            char* dot = strrchr(absolute_path, '.');
